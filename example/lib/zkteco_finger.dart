...
import 'package:zkteco_finger/finger_status.dart';
import 'package:zkteco_finger/finger_status_type.dart';
import 'package:zkteco_finger/zkteco_finger.dart';

class ZKTeciFinger extends StatefulWidget {
  const ZKTeciFinger({Key? key}) : super(key: key);

  @override
  _ZKTeciFingerState createState() => _ZKTeciFingerState();
}

class _ZKTeciFingerState extends State<ZKTeciFinger> {
...
AAA=''');
  ...
  Future<void> initPlatformState() async {
    ...
    try {
      platformVersion = await ZktecoFinger.platformVersion;
    } on PlatformException {
      platformVersion = 'Failed to get platform version.';
    }
    ZktecoFinger.imageStream.receiveBroadcastStream().listen(mapFingerImage);
    ZktecoFinger.statusChangeStream
    ...
    setState(() {
      _platformVersion = platformVersion ?? 'Failed to get platform version.';
    });
  }
      ...
      home: Scaffold(
        ...
        body: Row(
          children: <Widget>[
              ...
              child: Column(
                children: <Widget>[
                  Padding(
                    ...
                    child: Text('${fingerStatus?.statusType}'),
                  ),
                  RaisedButton(
                    onPressed: () async {
                      bool isSupported = await ZktecoFinger.isDeviceSupported();
                      setState(() {
                    ...
                    child: const Text('Is Device Supported'),
                  ),
                  RaisedButton(
                    onPressed: () async {
                      await ZktecoFinger.openConnection(isLogEnabled: true);
                    },
                    child: const Text('Open Connection'),
                  ),
                  RaisedButton(
                    onPressed: () async {
                      await ZktecoFinger.startListen(
                  ...
                  RaisedButton(
                    onPressed: () async {
                      await ZktecoFinger.verify(
                          userId: _registerationCodeController.text);
                    },
                    child: const Text('Verify Finger'),
                  ),
                  RaisedButton(
                    onPressed: () async {
                      await ZktecoFinger.clearFingerDatabase();
                    },
                    child: const Text('Clear finger\nDatabase'),
                  ),
                  RaisedButton(
                    onPressed: () async {
                      await ZktecoFinger.stopListen();
                    },
                    child: const Text('Stop Listening'),
                  ),
                  RaisedButton(
                    onPressed: () async {
                      await ZktecoFinger.closeConnection();
                    },
            ...
            //       RaisedButton(
            //         onPressed: () async {
            //           await ZktecoFinger.registerFinger(
            //               userId: _registerationCodeController.text,
            //               dataBase64: _biometricController.text);
            //         },
            //         child: const Text('Register User Biometric Base64 Data'),
            //       ),
            //     ],
            //   ),
            // ),
